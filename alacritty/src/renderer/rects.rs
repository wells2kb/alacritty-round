...
bitflags! {
    ...
    pub struct RoundCornerFlags: u32 {
        const TOP_RIGHT_ROUND_OUT    = 0b0000_0001;
        const TOP_RIGHT_ROUND_IN     = 0b0000_0011;
        const TOP_LEFT_ROUND_OUT     = 0b0000_0100;
        const TOP_LEFT_ROUND_IN      = 0b0000_1100;
        const BOTTOM_LEFT_ROUND_OUT  = 0b0001_0000;
        const BOTTOM_LEFT_ROUND_IN   = 0b0011_0000;
        const BOTTOM_RIGHT_ROUND_OUT = 0b0100_0000;
        const BOTTOM_RIGHT_ROUND_IN  = 0b1100_0000;
        const ALL_ROUND_OUT          = 0b0101_0101;
    }
}
...
impl RenderLine {
    /// Process a line into a rectangle that can be rendered.
    pub fn to_rect(
        self,
        ...
        let (position, thickness, rect_kind) = match flag {
            ...
            Flags::ROUND_BACKGROUND => (metrics.descent, size.cell_height(), RectKind::RoundBackground),
            _ => unimplemented!("Invalid flag for cell line drawing specified"),
        };

        Self::create_rect(size, metrics.descent, self.start, self.end, position, thickness, self.color, rect_kind, rects)
    }
    ...
    fn create_rect(
        ...
        if rect.kind == RectKind::RoundBackground {
            // Default to an round rectangle with no sharp/interior corners
            rect.corner_flags = RoundCornerFlags::ALL_ROUND_OUT;

            // Add interior corners and remove rounded corners when similar rectangles touch
            for prev_rect in rects {
                if rect.y == prev_rect.y + prev_rect.height
                && rect.color  == prev_rect.color
                && rect.x < prev_rect.x + prev_rect.width
                && rect.x + rect.width > prev_rect.x {

                    if rect.x < prev_rect.x {
                        prev_rect.corner_flags |= RoundCornerFlags::BOTTOM_LEFT_ROUND_IN;
                    }
                    else if rect.x > prev_rect.x {
                        rect.corner_flags |= RoundCornerFlags::TOP_LEFT_ROUND_IN;
                    }
                    else {
                        prev_rect.corner_flags &= !RoundCornerFlags::BOTTOM_LEFT_ROUND_IN;
                        rect.corner_flags &= !RoundCornerFlags::TOP_LEFT_ROUND_IN;
                    ...
                    if rect.x + rect.width > prev_rect.x + prev_rect.width {
                        prev_rect.corner_flags |= RoundCornerFlags::BOTTOM_RIGHT_ROUND_IN;
                    }
                    else if rect.x + rect.width < prev_rect.x + prev_rect.width {
                        rect.corner_flags |= RoundCornerFlags::TOP_RIGHT_ROUND_IN;
                    }
                    else {
                        prev_rect.corner_flags &= !RoundCornerFlags::BOTTOM_RIGHT_ROUND_IN;
                        rect.corner_flags &= !RoundCornerFlags::TOP_RIGHT_ROUND_IN;
...
impl RenderLines {
    ...
    pub fn rects(&self, metrics: &Metrics, size: &SizeInfo) -> Vec<RenderRect> {
        ...
        for (flag, lines) in &self.inner {
            for line in lines {
                let rect = line.to_rect(&mut rects, metrics, size, *flag);
                rects.push(rect);
            }
       }
    ...
    pub fn update(&mut self, cell: &RenderableCell) {
        ...
        self.update_flag(cell, Flags::ROUND_BACKGROUND);
    }
...
impl RectRenderer {
    pub fn new(shader_version: ShaderVersion) -> Result<Self, renderer::Error> {
        ...
        let round_background_program = RectShaderProgram::new(shader_version, RectKind::RoundBackground)?;
...
impl RectShaderProgram {
    pub fn new(shader_version: ShaderVersion, kind: RectKind) -> Result<Self, ShaderError> {
        ...
        let header = match kind {
            RectKind::RoundBackground => Some("#define DRAW_ROUND_BACKGROUND\n"),
